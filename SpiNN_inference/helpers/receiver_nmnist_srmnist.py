...
class Receiver_SRMNIST:
    ...
    def __init__(self,  receiver_conf):
        if not hasattr(self, 'initialized'):
            ...
            self.receive_labels = ["pop_inp", "pop_hidden_1", "pop_out"]
    ...
    def neuron_id_to_downScaled_map(self, neurons_id_array, out_size_y, out_size_x):
        ...
        sum_X, sum_Y = 0, 0
        for neurons_id in neurons_id_array:# exploring all the pos arrays captured at different time steps
    ...
    def extract_recordings_into_maps(self, pop_pos_data):
        ...
        if pop_pos_data['pop_label'] == 'pop_inp':
        ...
        if pop_pos_data['pop_label'] == 'pop_out':
            ...
            for n_id in neurons_id:
                self.wta_out[n_id] += 1#len(n_id)
        return


    def show_output(self):
        """
        Displays the accumulated input population activity map
        """
        cv2.imshow(f'Input Population from SpiNNaker', cv2.resize(self.temp_maps_downScaled, (200, 200)))
        cv2.waitKey(1)
        self.temp_maps_downScaled = np.zeros((self.W, self.H, 3), dtype=np.uint8)
    ...
    def process_output_events(self, time_tick_cnt, flag_ending_packet = False):
        ...
        if flag_ending_packet == True:
            if self.wta_out_accumulated.sum()>0:
                norm_wta_out_accumulated = (self.wta_out_accumulated - self.wta_out_accumulated.min()) / (self.wta_out_accumulated.max() - self.wta_out_accumulated.min())
                soft_max_like_P = norm_wta_out_accumulated/sum(norm_wta_out_accumulated)
                pred_time = time.time()
                ...
                with open(r"experiments\detections_test_validation.txt", "a") as f:
                    f.write(f"{pred},{pred_time},{bar.tolist()}\n")
                with open(r"experiments\spike_counts_test_validation.txt", "a") as f:
